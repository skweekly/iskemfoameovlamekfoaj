...
const api = 'http://twitchemotes.com/api_cache/v3/global.json';
const yn = 'https://yesno.wtf/#api'
...
client.on("ready", () => {
  ...
  client.user.setGame(`on ${client.guilds.size} servers | ?usefulcommands for help`);
  //message.reply("Hello, motherfuckers!")
});
...
client.on("message", async message => {
  ...
  //I am doing the same thing with switch cases now, because of it being more useful
  switch(command) {
      case 'suggestion':
      const args2 = message.content.slice(config.prefix.length).trim().split(/ +/g);
      const command2 = args.shift().toLowerCase();
      client.guilds.get('350647808111411200').channels.get("429352954231783436").send(args2)
      break;
    }
